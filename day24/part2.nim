...
proc boost_immune_system(army: Army, boost: int): Army =
  result = army
  for gr in result.mitems():
    gr.attack.damage += boost
...
proc main() =
  ...
  if boost == -1:
    echo "Error with the parameter. Provide a non-negative integer."
    quit(1)
  ...
  let
    ...
    immune_system_boosted = boost_immune_system(immune_system, boost)
    infection = init_army(sdInfection, infection_lines)
    (winner, part1_result) = war(immune_system_boosted, infection)
